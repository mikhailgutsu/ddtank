...
class TkSidePanel:
    ...
    """
    def __init__(self, master, width: int, border_width: int, on_grid_click, on_esp_click):
        self.width = int(width)
        self.border_width = int(border_width)
        
        self.on_grid_click = on_grid_click
        self.on_esp_click = on_esp_click
    ...
    def _build_buttons(self, parent):
        ...
        self._esp_default_fg  = self.esp_btn.cget("fg")
        try:
            self._esp_default_activebg = self.esp_btn.cget("activebackground")
        except tk.TclError:
    ...
    def _toggle_esp(self):
        ...
        if self.esp_on:
            self.esp_btn.config(text="ESP: ON", bg="#28a745", activebackground="#28a745", fg="white")
        else:
            self.esp_btn.config(text="ESP: OFF", bg=self._esp_default_bg,
                                activebackground=self._esp_default_activebg, fg=self._esp_default_fg)
        if callable(self.on_esp_click):
            self.on_esp_click(self.esp_on)
